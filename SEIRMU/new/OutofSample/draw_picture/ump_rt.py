...
def read_7models_data(idx):
    ...
    confirm_pred = read_pred_unemployment('./R/daily_week{}_R0.csv'.format(0, idx), alldates)
    ...
    for model_weeks in range(0, 25, 2):
        ...
        confirm_pred = read_pred_unemployment('./R/daily_week{}_R0.csv'.format(model_weeks, idx), alldates)
        pred_mob = pred_mob + confirm_pred
...
if __name__ == '__main__':
    ...
    for idx in range(1):
        ...
        paint_sca(pred_mob, pred_ump, idx)
    ...
    plt.title('{} and Unemployment Rate'.format('Rt'), fontsize=21)
    plt.savefig('./ump_rt/m_all.jpg', dpi=1200, pad_inches=0.0)
