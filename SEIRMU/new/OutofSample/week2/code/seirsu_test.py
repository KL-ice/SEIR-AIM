...
parser.add_argument('--iternum', nargs='+', type=int, default=[10, 10, 10, 10], help='ump, seir, mob, inf')
...
if_all = '_all' if args.iternum[3] >=51 else ''
infect_net = torch.load('../models/infection/' + 'infect{}_iter{}.pkl'.format(if_all, args.iternum[3])).eval()
# infect_net = torch.load('../models/infection/' + 'infect_iter{}.pkl'.format(0)).eval()
...
def test_lstm(N, x, m0, u0, i0, control, new7, ump7, mob7, last7, new_mu, new_sigma, inf_mu, inf_sigma, ump_data, mobility, oxc_data, infect_rate):
    ...
    for i in range(1, N):
        ...
        #if i < len(begindates) and begindates[i] == begin_change_date:
            #times += 1
            #value = _oxc[:, idx] / 7
        ...
        #if i >= len(begindates) or begindates[i] >= begin_change_date:
            # _oxc[:, idx] = torch.where((_oxc[:, idx] - times * value) < 0, torch.zeros_like(value), (_oxc[:, idx] - times * value))
        ...
        # if i < len(infect_rate):
            # inf = torch.FloatTensor([infect_rate[i]]).cuda()
        # else:
        #     inf = torch.FloatTensor([infect_rate[-1]-0.03*(i-len(infect_rate))]).cuda()
...
def main():
    ...
    out, mob_out, inf_out, true_inf, ump_out = test_lstm(len(alldates)+50, x, m0, u0, i0, control, new7, ump7, mob7, last7, new_mu, new_sigma, inf_mu, inf_sigma, ump_data[7:], mobility[6:], oxc_data, infect_rate)
    ...
    # csvdir = './csvs_diff_begin_frontier_ave/{}/{}/'.format(begin_change_date, args.comb)
    # csvdir = './csvs_diff_begin_ave/{}/{}/'.format(begin_change_date, idx)
